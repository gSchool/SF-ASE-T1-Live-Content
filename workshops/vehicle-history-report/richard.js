...
// DONE:
// declare a function that takes in a VIN as an argument - should not return anything
// find all the incidents for the vehicle with the VIN provided
// TODO: 
// sort the incidents list for the vehicle by reverse-chronological order
// print all the information in the format outlined in the requirements
...
const horizontalLine = ".".repeat(79);

function printHorizontalLine() {
  console.log(horizontalLine);
}

const reportTitle = "VEHICLE HISTORY";

function printHeader(vin) {
  printHorizontalLine();

  const titleSpacing = (horizontalLine.length - reportTitle.length - 2) / 2;
  // console.log(titleSpacing);

  console.log("|" + " ".repeat(titleSpacing) + reportTitle + " ".repeat(titleSpacing) + "|");
  printHorizontalLine();

  const vinSpacing = (horizontalLine.length - vin.length) / 2

  console.log(" ".repeat(vinSpacing) + vin + " ".repeat(vinSpacing));
  console.log(" ".repeat(vinSpacing) + "'".repeat(vin.length) + " ".repeat(vinSpacing));
  console.log();
}

const sortIncidents = function(in1, in2) {
  // console.log(new Date(car.date) - new Date(dino.date));
  return new Date(in2.date) - new Date(in1.date);
};

function printVehicleHistory(vin) {
  const vehicleIncidents = [];
  for(let v = 0; v < incidents.length; v++) {
    if(incidents[v].vin === vin) {
      vehicleIncidents.push(incidents[v]);
    }
  }
  ...
  vehicleIncidents.sort(sortIncidents);

  printHeader(vin);
  console.log("Incidents:");
  printHorizontalLine();

  for(let incident of vehicleIncidents) {
    console.log(incident.date, "-", incident.description);
    printHorizontalLine();
  }
}

const vinToPrint = "WBSWD9C58AP682105";
printVehicleHistory(vinToPrint);