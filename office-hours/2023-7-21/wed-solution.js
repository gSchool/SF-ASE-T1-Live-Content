...
function employeeInfo (employeeID) {
  // Inside of the function:
  // search over employees to find the employee object for the given employee id
  let employmentRecord;
  for (var i = 0; i < employees.length; i++) {
    if(employees[i].id === employeeID) {
      // store the details value from the found employee in a variable
      employmentRecord = employees[i];
    }
  }
      
  // var employeeDetails = employees[i].details;
  ...
  for (var j = 0; j < people.length; j++) {
    // store the first and last from the found person in a couple variables
    // var employeeName = people[j].first_name + " " + people[j].last_name;
    //console.log(employeeName);

    if(people[j].id === employmentRecord.details) {
      employeeName = people[j].first_name + " " + people[j].last_name;
    }
  }

  console.log(employeeName, "-", employmentRecord.paytype);

  // return employeeName + " " + employees[i].paytype;
}

function printEmpDeets(empID, empArray, pplArray){
  let empObject = {};
  let personObj = {};
  for(const empObj of empArray){
      if(empObj.id === empID){
          empObject = empObj;
          break;
      }
  ...
  for(const pplObj of pplArray){
      if(pplObj.id === empObject.details){
          personObj = pplObj;
          break;
      }
  }

  console.log(`${personObj.first_name} ${personObj.last_name} - ${empObject.paytype}`);
}

employeeInfo(4);
printEmpDeets(4, employees, people);